...
import { HomeIcon, PaperAirplaneIcon, ChatBubbleLeftIcon, PlayCircleIcon, PhoneIcon, XMarkIcon, ChevronDownIcon } from "@heroicons/react/24/solid";
...
const callsToAction = [
    { name: "See Demo Booking", href: "#", icon: PlayCircleIcon, id: 1 },
    { name: "Contact Support", href: "#", icon: PhoneIcon, id: 2 }
];
        ...
        <nav className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-0" aria-label="Global">
            ...
            <div className="flex lg:flex-1">
                <Link href="/" className="-m-1.5 p-1.5">
                    <span className="sr-only">Booking.com</span>
                        <h1 className="h-5 w-auto font-extrabold text-white">Booking.com</h1>
                </Link>
            </div>

            {/* Menu icon */}
            <div className="flex lg:hidden">
                <button type="submit" onClick={() => setMobileMenuOpen(true)} className="-m-2.5 inline-flex items-center justify-center rounded-md p-2.5 text-white">
                            ...
                            <div className="grid grid-cols-2 divide-x divide-gray-900/5 bg-gray-50">
                                {
                                    callsToAction.map(item => (
                                        <a href={item.name} key={item.id} className="flex items-center justify-center gap-x-2.5 p-3 text-sm font-semibold leading-6 text-[#013894] hover:bg-gray-100">
            ...
            <div className="hidden lg:flex lg:flex-1 lg:justify-end">
                <a href="#" className="text-sm font-semibold leading-6 text-white">
                    Log in <span ari-hidden="true">&rarr;</span>
                </a>
            </div>
        </nav>
            ...
            <Dialog.Panel className="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-[#013894] px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
                <div className="flex items-center justify-between">
                    <a href="#" className="-m-1.5 p-1.5">
                        <span className="sr-only">Booking.com</span>
                        <h1 className="text-white font-extrabold">Booking.com</h1>
                    </a>
                                        ...
                                        <>
                                            ...
                                            <Disclosure.Panel className="mt-2 space-y-2">
                                                {[...products, ...callsToAction].map((item) => (
                                                    <Disclosure.Button key={item.id} as="a" href={item.href} className="block rounded-lg py-2 pl-6 pr-3 text-sm font-semibold leading-7 text-white hover:bg-blue-800">
                                                        {item.name}
                                                    </Disclosure.Button>
                                                ))}
                                            </Disclosure.Panel>
